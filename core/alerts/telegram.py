...
class TelegramAlerts:
    ...
    def format_fill(self, fill: Fill) -> str:
        ...
        return (
            ...
            f"FairProb: {fill.signal.fair_prob:.3f}  Edge: {edge_pct:+.1f}%  Mode: {fill.mode}"
        )
    ...
    def format_equity_scan(
        ...
        lines = [
            f"[EQUITY SCAN] {len(swing_candidates)} swing / {len(core_candidates)} core candidates",
        ...
        if swing_candidates:
            ...
            for c in swing_candidates:
                ...
                lines.append(
                    f"  {c.instrument.ticker} [{c.event_type.value}]{days}"
                    f" — {c.evidence[:70]}"
                )
        if core_candidates:
            lines.append("\nCore quality screen passed:")
            for c in core_candidates:
                lines.append(f"  {c.instrument.ticker} score={c.score:.2f} — {c.evidence[:70]}")
        return "\n".join(lines)
    ...
    def format_equity_open(self, rec: Any, fill: Any) -> str:
        ...
        thesis = rec.thesis[:140] + "..." if len(rec.thesis) > 140 else rec.thesis
        return (
    ...
    def format_equity_exit(
        ...
        reason_label = reason_map.get(exit_signal.reason, exit_signal.reason.upper())
        s = portfolio_stats
        lines = [
            f"[PAPER EXIT] {ticker} — {reason_label} ({result})",
            f"Entry: ${entry_price:.2f} -> Exit: ${exit_signal.exit_price:.2f}",
            f"P&L: {pnl:+.2f} ({pnl_pct:+.1f}%)",
            f"---",
            f"Portfolio: {s['closed_count']} closed ({s['wins']}W {s['losses']}L)"
            f" | Win: {s['win_rate'] * 100:.0f}%"
            f" | Realized: {s['realized_pnl']:+.2f}",
        ]
        if s["open_count"] > 0:
            lines.append(f"Open: {s['open_count']} position(s) | Unrealized: {s['unrealized_pnl']:+.2f}")
        return "\n".join(lines)
    ...
    def format_equity_portfolio(self, stats: dict[str, Any]) -> str:
        ...
        lines = [
            f"[PORTFOLIO] Equity paper account",
            f"Open: {s['open_count']} position(s)",
            f"Closed: {s['closed_count']} trade(s) ({s['wins']}W {s['losses']}L)"
            f" | Win rate: {s['win_rate'] * 100:.0f}%",
            f"Realized P&L: {s['realized_pnl']:+.2f}",
        ]
        if s["open_count"] > 0:
            lines.append(f"Unrealized P&L: {s['unrealized_pnl']:+.2f}")
            lines.append(f"Total P&L: {total_pnl:+.2f}")
            lines.append("\nOpen positions:")
            for pos in s.get("open_positions", []):
                unrl = pos.get("unrealized_pnl") or 0.0
                mark = pos.get("mark_price") or pos.get("entry_price", 0.0)
                lines.append(f"  {pos['ticker']}  mark=${mark:.2f}  unrl={unrl:+.2f}")
        return "\n".join(lines)
