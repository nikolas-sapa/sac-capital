...
def _passing_matrix() -> list[list[float]]:
    """4 noise trials + 1 trial with a real, persistent edge — clears the
    default PBO/DSR thresholds. Same construction as the overfitting.py
    self-check.
    """
    rng = np.random.default_rng(1)
    noise = rng.normal(0.0, 1.0, size=(500, 4))
    signal = rng.normal(0.35, 1.0, size=500)
    return np.column_stack([noise, signal]).tolist()
...
def test_promote_passes_all_gates(tmp_path):
    ...
    promoted = promoter.try_promote(
        _variant(), tracker, trial_returns_matrix=_passing_matrix(), live_paper_trade_count=5
    )
...
def test_promote_blocked_by_cooldown(tmp_path, monkeypatch):
    ...
    # First promotion
    promoter.try_promote(
        _variant("v1"), tracker, trial_returns_matrix=_passing_matrix(), live_paper_trade_count=0
    )

    # Second attempt immediately → blocked by cooldown
    result = promoter.try_promote(
        _variant("v2"), tracker, trial_returns_matrix=_passing_matrix(), live_paper_trade_count=0
    )
    assert result is False


def test_promote_blocked_by_overfitting_check(tmp_path):
    tracker = ForwardPaperTracker(tmp_path / "fp.db")
    _add_winning_trades(tracker, 10)
    gate = KillGate(min_trades=5)
    promoter = AutoPromoter(gate=gate, cooldown_days=0, min_live_paper_trades=0)

    # Many noise trials, little data — indistinguishable from picking the
    # luckiest of many trials (see equities/eval/overfitting.py self-check).
    rng = np.random.default_rng(0)
    noisy_matrix = rng.normal(0.0, 1.0, size=(60, 50)).tolist()

    result = promoter.try_promote(
        _variant(), tracker, trial_returns_matrix=noisy_matrix, live_paper_trade_count=0
    )
    assert result is False
    assert promoter.current_params() is None
    rejection = promoter.rejection_history()[-1]
    assert rejection.stage == "overfitting"
...
def test_rollback_fails_with_one_promotion(tmp_path):
    ...
    promoter = AutoPromoter(gate=gate, cooldown_days=0, min_live_paper_trades=0)
    promoter.try_promote(
        _variant(), tracker, trial_returns_matrix=_passing_matrix(), live_paper_trade_count=0
    )
